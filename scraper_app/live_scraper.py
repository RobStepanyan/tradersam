...
# CollectAllAssetsLive.funds()
    def commodities():
        """  
        If the market is closed (redClockIcon) then check whether "before live data" for previous day 
        is available if yes then collect "after live", otherwise collect only "before live data"
        
        If the market is open (greenClockIcon) then collect "live data" and every 1 minutes pass 
        it to "historical 1D", every 5m to "historical 5D"
        """
        ...
        driver.get(url)
        print('Commodities: table link is visited')
        while True:
            soup = BeautifulSoup(driver.page_source, 'html.parser')
            table = soup.find('table', class_='genTbl closedTbl crossRatesTbl').tbody
            print(f'Commodities: table is updated ({datetime.now().strftime("%d/%m/%Y %H:%M:%S")})')
            for tr in table.find_all('tr'):
                ...
                if time_icon == 'greenClockIcon':
                    print('Commodities: greenClockIcon')
                    # Update Live Data
                    if month in '  ':
                        month = None
                    else:
                    ...
                    print('Commodities: saved live data')
                    ...
                    print('Commodities: redClockIcon')
                    # check whether "before live data" for previous day is available
                    ...
                    if last_obj_after == None:
                        after_is_newer_than_before = False
                    elif last_obj_after.date>last_obj_before.date:
                        after_is_newer_than_before = True

                    if (last_obj_before_count==0 and last_obj_after_count==0) or after_is_newer_than_before:
                        # Save BeforeLive
                        ...
                        print('Commodities: saved before live')
                    else:
                        # Save AfterLive
                        header={'User-Agent':'Mozilla/5.0 (Windows NT 6.1; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/41.0.2227.0 Safari/537.36'}
                        request = requests.get(link, headers=header)
                        ...
                        while True:
                            try:
                                ...
                                settlement_day = datetime.date.strptime(settlement_day, '%m/%d/%Y')
                                last_roll_day = soup.find('span', text='Last Rollover Day').find_next_sibling('span').get_text()
                                last_roll_day = datetime.date.strptime(last_roll_day, '%m/%d/%Y')
                                break
                            except Exception as e:
                                exc_type, exc_obj, exc_tb = sys.exc_info()
                                fname = os.path.split(exc_tb.tb_frame.f_code.co_filename)[1]
                        ...
                        CommodityAfterLive(
                            short_name=tds[0], link=link,
                            date=now.date(),
                            one_year_rng=one_year_rng, one_year_chg=one_year_chg,
                            months=months, 
                            settlement_day=settlement_day, last_roll_day=last_roll_day
                        ).save()
                        print('Commodities: saved after live')
                else:
